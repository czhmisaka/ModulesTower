    ...
    <el-card class="search-config" v-if="queryItemOptions&&queryItemOptions.length>0"
      ...
      <el-form @submit.native.prevent :inline="true" :model="queryOption" label-suffix=":" label-width="120px"
        ...
        <el-form-item v-for="(item,index) in queryItemOptions" :key="item.prop+index" :label="item.label">
          ...
          <el-select style="width:auto" v-if="item.type==='inputList'" v-model="queryOption[item.prop]" multiple
            clearable filterable allow-create default-first-option collapse-tags :placeholder="item.label"
            @change="batchInput('queryOption',item.prop)">
    ...
    <el-card class="search-result">
      ...
      <el-header style="padding-left:0px;height: auto;padding-right:0px">
        <span v-for="(item,index) in actionButtonList" v-bind:key="index" style="display:inline-block">
          <el-upload :on-success="(e)=>{uploadExcelTemplate(e,item)}" class="action-button" :show-file-list="false" @
            v-if="item.actionType=='uploadExcelTemplate'" :action="apiUpload" :accept="item.accept" list-type="none">
            <el-button :type="item.type" :icon="item.icon">{{item.name}}</el-button>
          </el-upload>
          <el-button v-else class="action-button" :type="item.type" :icon="item.icon" @click="clickActionButton(item)"
            :disabled="item.isDisable?selectedList.length>0?false:true:false">
            {{item.name}}
          </el-button>
        </span>
        <el-button v-if="config.moduleConfig&&config.moduleConfig.feature&&config.moduleConfig.feature.positionListMap"
          style="float:right;margin:0px 0px 0px 10px;" class="action-button" type="primary" icon="el-icon-location"
      ...
      <el-table ref="tableController" :data="dataList.data" @selection-change="selectPosition" border stripe
        v-loading="loading" style="cursor:default" @cell-dblclick="showDetail"
        :row-style="{'min-height':'60px','min-width':'100px'}" :fit='true'>
        <el-table-column type="selection" align="center" fixed='left' style="min-height:100px">
        </el-table-column>
        <div v-for="(item) in tableColumnList" v-bind:key="item.label">
          <el-table-column v-if="item.selectable" :filters="filtersList[item.prop]" :filter-method="filterByProp"
            width="160" :label="item.label" :prop="item.prop" :sortable="item.sortable">
          ...
          <el-table-column v-if="item.backUpProp&&!item.selectable" width="180" :label="item.label" :prop="item.prop"
            :sortable="item.sortable">
            <template slot-scope="scope">
              <span v-if="item.template">
                <span v-for="(it,ind) in item.template" v-bind:key="ind+''">
                  <span v-if="!it.element">{{it.key?(scope.row[it.key]?scope.row[it.key]:it.value):it.value}}</span>
                  <span v-if="it.element=='tag'">
                    <el-tag :type="it.elementType">
                      {{it.return?it.return(scope.row):it.value?it.value:scope.row[it.key]}}</el-tag>
                  </span>
                  <span v-if="it.element == 'func'">
                    {{it.return(scope.row)}}
                  </span>
                </span>
              </span>
              <span v-else>
                <span v-for="(it,ind) in item.backUpTemplate" v-bind:key="ind+''">
                  <span v-if="!it.element">{{it.key?(scope.row[it.key]?scope.row[it.key]:it.value):it.value}}</span>
                  <span v-if="it.element=='tag'">
                    <el-tag :type="it.elementType?it.elementType(scope.row):'primary'"
          ...
          <el-table-column v-if="!item.backUpProp&&!item.selectable" width="200" :label="item.label" :prop="item.prop"
            :sortable="item.sortable">
            <template slot-scope="scope">
              <span v-if="item.template">
                <span v-for="(it,ind) in item.template" v-bind:key="ind+''">
        ...
        <el-table-column fixed="right" label="操作" v-if="config.apiConfig.dealTool&&config.apiConfig.dealTool.length>0"
          :width="config.apiConfig.dealTool_width||'80px'">
          <template slot-scope="scope" style="margin-right:10px;display:inline-block;display:inline-block;width:auto;">
            <div v-for="(it,ind) in config.apiConfig.dealTool" :key="it.name+ind" :style="it.style">
              <el-button size="mini"
                :style="'display:inline-block;'+(config.apiConfig.dealTool.length>1&&ind<config.apiConfig.dealTool.length-1?'margin-bottom:10px':'')"
                :type="it.type" @click="(e)=>{dealTool_apiButton(scope,it,ind)}">
                {{it.name}}
              </el-button>
            </div>
          </template>
      ...
      <!-- 分页组件 -->
      <el-pagination class="page-footer" @size-change="handleSizeChange" @current-change="handleCurrentChange"
        current-page.sync="0" :page-sizes="[10, 20, 30, 50]" :page-size="10" layout="sizes, prev, pager, next"
        :total="dataList.totalCount">
      </el-pagination>
      <!-- 分页组件 -->

      <!-- 导出组件 -->
      <ExportTable ref="EXPORTTABLE" :config="config" @needRefresh='needRefresh' :selectedLists="selectedList"
        :queryOption="queryOption"></ExportTable>
      <!-- 导出组件 -->

      <!-- DealElement 组件 -->
      <DealElement ref="DEALELEMENT" :config="config" :actionButtonCell="actionButtonCell" @needRefresh='needRefresh'
        :selectedLists="selectedList" :parentComponentQueryOption="queryOption">
      </DealElement>
      <!-- DealElement 组件 -->
      ...
      <el-drawer @close='beforeClose' :title="showDetailControl.id+''" :visible.sync="showDetailControl.show"
        ...
        <div style="width:100%;height:100%;overflow-x:hidden;padding:30px;z-index:10000" class="scrollStyle">
          <positionDetail v-if="config.moduleConfig.type&&config.moduleConfig.type=='point'"
            :positionIdFromProp='showDetailControl.id'></positionDetail>
          <AssetsCabinetListDetail
            v-if="showDetailControl.show&&showDetailControl.row&&config.moduleConfig.type&&config.moduleConfig.type=='listDetail'"
            :tableDetail="config.moduleConfig.listDetail_dataKey?showDetailControl.row[config.moduleConfig.listDetail_dataKey]:[]"
            :id="config.moduleConfig.listDetail_IdKey?showDetailControl.row[config.moduleConfig.listDetail_IdKey]:''"
            :idKey="config.moduleConfig.listDetail_IdKey" :api="config.moduleConfig.listDetail_api"
...
<script>
  ...
  function handle(e) {
    ...
    if (tableBodyDomTop > window.innerHeight || tableBodyWrapDom.classList.contains('is-scrolling-none')) {
      ...
      if (e && e.target && e.target.className && e.target.className.split(' ')[0] == 'el-table__body-wrapper') {
  ...
  export default {
    ...
    data() {
      return {
        ...
        selectedList: [], // 已选择的选项
        actionButtonCell: {}, // 当前正在处理的按钮事件详情
    ...
    methods: {
      ...
      async importConfigs(path) {
        ...
        this.tableColumnList = this.config.tableColumn
      ...
      async onSearch(isNew = true) {
        if (isNew) {
          this.queryOption.pageSize = this.pageSize
          this.queryOption.pageNum = 0
        }
        this.loading = true
        let data = {}
        let that = this
        ...
        if (Api.dataDealFunc) {
          data = Api.dataDealFunc(that)
        } else {
          data = {
            data: {
              ...this.config.apiConfig.onSearch.defaultData,
              ...this.queryOption,
            }
          }
        }
        let res = await this.$axios.post(this.$api[this.config.apiConfig.root][this.config.apiConfig.onSearch.api],
          this.config.apiConfig.onSearch.data ? this.config.apiConfig.onSearch.data(that) : data)
        if (res.code != '0000') return;
        this.loading = false;
        let dataList = res.data.data ? res.data.data.data ? res.data.data : res.data : {};
        dataList.data = dataList.data ? dataList.data.data ? dataList.data.data : dataList.data : []
        if (dataList.data && dataList.data.length > 0) {
          dataList.data.forEach(item => {
            let that = this
            item = this.$utils.commonDataDealForNormalList(item, Api.replace, that)
          })
        } else {
          dataList.data = []
        }
      ...
      async uploadExcelTemplate(e, action) {
        if (action.asyncSign)
          return this.addTaskToUploadFileStatus({
            ...action.api.pre_data,
            fileName: (action.api.bindTimestamp ? new Date().getTime() + '_' : '') + e.data[0].fileName,
      ...
      async startUploadFileProcess(that = this) {
        ...
        if (uploadFileStatus.map(x => {
          ...
          uploadFileStatus.forEach(async (x, index) => {
            if (x.data && x.data.fileName) {
              if (!that.$api[x.api.root][x.api.api]) return console.log(that.$api[x.api.root], x.api.api);
              ...
              if (data && data.split(':').length > 1) {
                if (data[0] != 'h') {
                  uploadFileStatus[index] = null
                  this.$alert(data, '导入错误')
        ...
        let length = uploadFileStatus.map(x => {
          if (x != null && x.data && x.data.fileName != null) return x
          else return null
        ...
        that.$utils.setJsonAsStrToJsonByKey('AsyncTaskStatus_' + this.path.split('/'), that.uploadFileStatus)
      },
