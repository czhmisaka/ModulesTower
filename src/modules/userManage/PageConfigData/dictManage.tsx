 ...
 * @LastEditors: CZH
 * @LastEditTime: 2023-11-07 15:22:33
 * @FilePath: /lcdp_fe_setup/src/modules/userManage/PageConfigData/dictManage.tsx
 */
import {
    gridCellMaker,
    gridSizeMaker,
...
}, ['/web/sys/dict/insert'], '编辑字典按钮')

const 删除字典按钮 = btnMaker('删除', btnActionTemplate.Function, {
    icon: 'Delete',
    elType: 'danger',
    function: async (that, data) => {
        if (await dobuleCheckBtnMaker('删除字典', '删除字典' + data.name).catch(x => false)) {
            let res = await post('/web/sys/dict/delete', { id: data.id })
            repBackMessageShow(that, res)
        }
    }
...
字典页面配置数据列.push(
    tableCellTemplateMaker(
        "操作",
        "actionaction",
        actionCell([删除字典按钮, 编辑字典按钮, 查看字典数据], {
            fixed: "right",
        })
    )
);
...
export const dictManageBtnList = [新增字典按钮, 编辑字典按钮, 删除字典按钮, 查看字典数据, ...dictDataManageBtnList,
    roleBtnMaker(["/web/sys/dict/page"], '查询字典列表')
];
