 ...
 * @LastEditors: CZH
 * @LastEditTime: 2023-11-08 18:45:24
 * @FilePath: /lcdp_fe_setup/src/modules/userManage/PageConfigData/adminManage.tsx
 */
...
import { btnMaker, dobuleCheckBtnMaker, roleBtnMaker, } from "../component/searchTable/drawerForm";
...
const 新增管理员弹窗 = btnMaker('新增管理员', btnActionTemplate.OpenDrawer, {
  ...
  drawerProps: {
    title: '新增管理员',
    queryItemTemplate: [
      tableCellTemplateMaker('用户', 'id', searchCell(formInputType.searchList, {
        inputOptions: {
          type: 'string',
          multiple: false,
          remoteMethod: async (query) => {
            let res = await post('/web/usc/user/page', { name: query, pageSize: 20, pageNumber: 0 })
...
const 取消管理员权限 = btnMaker('解除',
  btnActionTemplate.Function, {
  icon: "Delete",
  elType: "danger",
  function: async (that, data) => {
    if (await dobuleCheckBtnMaker('解除权限', `解除用户【${data.name}】的管理员权限吗`).catch(x => false)) {
...
export const adminManage = async () => {
  ...
  const 管理员列表 = new SearchCellStorage([
    ...
    tableCellTemplateMaker('部门', 'unitNames'),
    ...
    gridCellMaker(
      ...
      },
      {
        props: {
          ...
          autoSearch: true,
          btnList: 搜索区域操作栏,
          showItemTemplate: 管理员列表.getAll(['adminFlag']),
...
export const adminManageBtnList = [
  新增管理员弹窗,
  取消管理员权限,
  roleBtnMaker(["/web/usc/user/page/admin"], '查询管理员列表')
]
