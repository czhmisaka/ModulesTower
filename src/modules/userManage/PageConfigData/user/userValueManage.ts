 ...
 * @LastEditors: CZH
 * @LastEditTime: 2023-11-15 17:44:15
 * @FilePath: /lcdp_fe_setup/src/modules/userManage/PageConfigData/user/userValueManage.ts
 */
import {
  gridCellMaker,
  gridSizeMaker,
    ...
    searchCell(formInputType.searchList, {
      funcInputOptionsLoader: async (that) => {
        ...
        attr["remoteMethod"] = async (query) => {
          let res = await post("/web/sys/dict/page", {
            name: query,
          });
...
const 新增用户字段按钮 = btnMaker(
  ...
  ["/web/usc/customize/field/insert", "/web/sys/dict/page"],
  "新增用户字段按钮"
);
...
const 编辑用户字段按钮 = btnMaker(
  ...
  ["/web/usc/customize/field/update", "/web/sys/dict/page"],
  "编辑用户字段按钮"
);
...
export const userValueManageBtnList = [
  ...
  删除用户字段按钮,
  新增用户字段按钮,
  roleBtnMaker([valueListApi], "查询接口"),
];
