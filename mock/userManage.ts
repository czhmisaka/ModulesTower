 ...
 * @LastEditors: CZH
 * @LastEditTime: 2023-07-19 16:03:16
 * @FilePath: /lcdp_fe_setup/mock/userManage.ts
 */
// 模拟后端动态生成路由
  ...
  for (let x in obj) {
    switch (obj[x]) {
      ...
      case fakeDataType.dataString:
        backData[x] =
          new Date(Math.random() * new Date().getTime()).getTime() + "";
        break;
      ...
      case fakeDataType.mail:
        backData[x] = `${randomEnglishString(
          Math.random() * 30
        )}@${randomEnglishString(Math.random() * 10)}.fake`;
        break;
...
export const unitlistMaker = (numbers) => {
  ...
  unitList.map((x, i) => {
    if (i > 0 && i % 2 == 0) unitList[i].parentId = unitList[i - 1].id;
  });
  return unitList;
};
